							...
							if (typeof sentence === 'string') {
								...
								switch (Object.keys(sentence)[0]) {
									...
									case "h4":
										key++
										return (
											<h4 key={key} className={st.h4}>{sentence.h4}</h4>
										)
									case "link":
										key++
										return (
											<a key={key} className={st.link}
												href={"https://" + sentence.link}
												title={"Link to " + sentence.link}
												rel="noopener noreferrer"
												target="_blank">{Object.values(sentence)}</a>
										)
									case "mail":
										key++
										return (
									...
									default:
										key++
										return (
											<h3 key={key} className={st.h3}>{sentence.h3}</h3>
										)
								}
							}
						} else {
